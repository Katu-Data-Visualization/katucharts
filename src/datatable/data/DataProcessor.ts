...
export class DataProcessor {
  ...
  private applyFilters(
    ...
    return data.filter(row => {
      for (const [field, filterVal] of activeFilters) {
        ...
        /**
         * A custom filterFn takes precedence.
         */
        if (col.filterFn) {
          if (!col.filterFn(row, field, filterVal)) return false;
          continue;
        }
  ...
  private matchesFilter(value: any, filterVal: any, col: InternalColumnDef): boolean {
    ...
    switch (filterType) {
      ...
      case 'boolean': {
        if (typeof filterVal === 'boolean') return Boolean(value) === filterVal;
      ...
      case 'faceted': {
        if (Array.isArray(filterVal)) {
          return filterVal.length === 0 || filterVal.map(String).includes(String(value));
        }
        return String(value) === String(filterVal);
      }
  ...
  private applySorting(
    ...
    dataRows.sort((a, b) => {
      for (const { column, direction } of state.sort) {
        ...
        const valA = col ? resolveCellValue(a._data, col) : a._data[column];
        const valB = col ? resolveCellValue(b._data, col) : b._data[column];
  ...
  private computeAggregates(
    ...
    for (const col of columns) {
      ...
      const values = data.map(row => resolveCellValue(row, col)).filter(v => v != null);
