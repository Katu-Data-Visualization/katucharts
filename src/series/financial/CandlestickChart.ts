...
export class CandlestickChart extends BaseSeries {
  ...
  protected getCandleStyle(ctx: CandleStyleContext): { fill: string; stroke: string; wick: string } {
    const line = ctx.isUp ? ctx.upLineColor : ctx.downLineColor;
    return { fill: ctx.isUp ? ctx.upColor : ctx.downColor, stroke: line, wick: line };
  }
  ...
  render(): void {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const style = this.getCandleStyle({
        open, high, low, close, isUp, index: i, data,
        upColor, downColor, upLineColor, downLineColor,
      });
      const bodyColor = style.fill;
      const lineColor = style.stroke;

      const g = this.group.append('g')
        .attr('class', this.getCandleClass())
        .style('cursor', this.config.cursor || 'pointer');
      candles.push(g);

      const wick = g.append('line')
        .attr('x1', cx).attr('x2', cx)
        .attr('stroke', style.wick)
