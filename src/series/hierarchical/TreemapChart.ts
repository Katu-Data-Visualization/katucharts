...
/**
 * Parses a label template containing <br/> line breaks and <b style="font-size: ...">
 * tags into an array of lines, each containing styled text segments.
 */
function parseLabelSegments(
  raw: string,
  ...
  return lines.map(line => {
    ...
    const tagRe = /<b(?:\s+style="([^"]*)")?\s*>([\s\S]*?)<\/b>/gi;
    let lastIdx = 0;
    let m: RegExpExecArray | null;
    while ((m = tagRe.exec(line)) !== null) {
      if (m.index > lastIdx) {
        const plain = stripHtmlTags(line.slice(lastIdx, m.index));
        if (plain) segments.push({ text: plain, fontSize: defaultFontSize, fontWeight: defaultFontWeight });
      }
      const styleStr = m[1] || '';
      const fontSizeMatch = styleStr.match(/font-size\s*:\s*([^;"]+)/i);
      const weightMatch = styleStr.match(/font-weight\s*:\s*([^;"]+)/i);
      const inner = stripHtmlTags(m[2]);
      if (inner) {
        segments.push({
          text: inner,
          fontSize: fontSizeMatch ? fontSizeMatch[1].trim() : defaultFontSize,
          fontWeight: weightMatch ? weightMatch[1].trim() : 'bold',
        });
      }
      lastIdx = m.index + m[0].length;
    }
    if (lastIdx < line.length) {
      const plain = stripHtmlTags(line.slice(lastIdx));
      if (plain) segments.push({ text: plain, fontSize: defaultFontSize, fontWeight: defaultFontWeight });
    }
    return segments;
  });
...
export class TreemapChart extends BaseSeries {
  ...
  render(): void {
    ...
    const colorAxisClasses: Array<{ from?: number; to?: number; color: string }> = (this.config as any).colorAxis?.dataClasses || [];
    const dataClassPalette = colorAxisClasses.map(c => c.color);
    ...
    const getNodeColor = (d: any, i: number): string => {
      ...
      if (dataClassPalette.length > 0) {
        return dataClassPalette[i % dataClassPalette.length];
      }
      if (colorByPoint) {
    ...
    const cells = this.group.selectAll('.katucharts-treemap-cell')
      ...
      .attr('fill', (d: any, i: number) => {
        const c = getNodeColor(d, i);
        d._color = c;
        return c;
      })
      .attr('stroke', (d: any) => {
        const lc = getLevelConfig(d.depth);
        return lc?.borderColor || this.config.borderColor || this.autoBorderColor();
      })
  ...
  private renderLabels(leaves: any[], getLevelConfig: (d: number) => TreemapLevelOptions | undefined): void {
    ...
    const labelColorOverride = dlCfg.color || (dlCfg.style?.color as string) || undefined;
    const defaultFontWeight = (dlCfg.style?.fontWeight as string) || 'normal';
    const align = (dlCfg.align as string) || 'left';
    const verticalAlign = (dlCfg.verticalAlign as string) || 'top';
    const padX = (dlCfg.x as number) ?? 4;
    const padY = (dlCfg.y as number) ?? 4;

    this.group.selectAll('.katucharts-treemap-label').remove();
    ...
    leaves.forEach((d: any, i: number) => {
      ...
      let rawText = '';
      if (dlCfg.formatter) {
        rawText = dlCfg.formatter.call({
          point: d.data, series: { name: this.config.name },
          x: d.data.x, y: d.data.y ?? d.data.value,
        });
      } else {
        const fmt = dlCfg.format || (dlCfg as any).pointFormat;
        if (fmt) {
          rawText = templateFormat(fmt, {
            point: d.data, series: { name: this.config.name },
            x: d.data.x, y: d.data.y ?? d.data.value,
          });
        } else {
      ...
      const segments = parseLabelSegments(rawText, defaultFontSize, defaultFontWeight);

      let anchorX: number;
      if (align === 'right') anchorX = d.x1 - padX;
      else if (align === 'center') anchorX = (d.x0 + d.x1) / 2;
      else anchorX = d.x0 + padX;
      const textAnchor = align === 'right' ? 'end' : align === 'center' ? 'middle' : 'start';

      let anchorY: number;
      if (verticalAlign === 'bottom') anchorY = d.y1 - padY;
      else if (verticalAlign === 'middle') anchorY = (d.y0 + d.y1) / 2;
      else anchorY = d.y0 + padY;

      const textEl = this.group.append('text')
        .attr('class', 'katucharts-treemap-label')
        .attr('x', anchorX)
        .attr('text-anchor', textAnchor)
        .attr('fill', readableTextColor(d._color, labelColorOverride))
        .style('pointer-events', 'none');

      let currentY = anchorY;
      segments.forEach((line, lineIdx) => {
        if (line.length === 0) return;
        const lineFontPxs = line.map(s => parseFontSizePx(s.fontSize));
        const lineHeightPx = Math.max(...lineFontPxs) * 1.15;
        currentY += lineHeightPx;
        line.forEach((seg, segIdx) => {
          const tspan = textEl.append('tspan')
            .attr('font-size', seg.fontSize)
            .attr('font-weight', seg.fontWeight)
            .text(seg.text);
          if (segIdx === 0) {
            tspan.attr('x', anchorX).attr('y', currentY);
          }
        });
        void lineIdx;
      });
