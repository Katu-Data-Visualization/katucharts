...
export class SunburstChart extends BaseSeries {
  ...
  render(): void {
    ...
    if (rootRadius > 15) {
      ...
      if (rootLabel) {
        ...
        const labelColor = readableTextColor(rootColor, dlStyle.color);
  ...
  private renderLabels(g: any, nonRoot: any[], levels: LevelConfig[]): void {
    ...
    const colorByPointLevel = levels.find(l => l.colorByPoint)?.level;
    const baseLevel = colorByPointLevel ?? 1;
    const groupColor = (d: any) => {
      let n = d;
      while (n.depth > baseLevel && n.parent) n = n.parent;
      return n._color;
    };

    /**
     * Resolves label text color from the base-palette ancestor (rather than
     * the brightness-varied fill color) so every slice in a color group gets
     * consistent dark or light text. This keeps the entire branch readable
     * instead of siblings straddling the contrast threshold flipping between
     * dark and light text.
     */
    const resolveTextColor = (d: any) => {
      const lc = levels.find(l => l.level === d.depth);
      const override = lc?.dataLabels?.style?.color || globalStyle.color;
      return readableTextColor(groupColor(d), override);
    };
    ...
    g.selectAll('.katucharts-sunburst-label')
      ...
      .attr('font-weight', 'bold')
      .attr('fill', (d: any) => resolveTextColor(d))
      .style('paint-order', 'stroke fill')
      .attr('stroke', (d: any) => {
