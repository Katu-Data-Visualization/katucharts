...
export class GaugeChart extends BaseSeries {
  ...
  render(): void {
    ...
    const cy = isSemicircle ? plotArea.height * 0.62 : plotArea.height / 2;
    const radius = isSemicircle
      ? Math.min(plotArea.width / 2, cy) - 20
      : Math.min(plotArea.width, plotArea.height) / 2 - 20;

    const min = ((this.config as any)._yAxis ?? (this.config as any).yAxis)?.min ?? this.config.min ?? 0;
    const max = ((this.config as any)._yAxis ?? (this.config as any).yAxis)?.max ?? this.config.max ?? 100;
  ...
  private renderTickMarks(g: any, radius: number, startAngle: number, endAngle: number, min: number, max: number, bandThickness?: number): void {
    const yAxisCfg = ((this.config as any)._yAxis ?? (this.config as any).yAxis) || {};
...
export class SolidGaugeChart extends BaseSeries {
  ...
  render(): void {
    ...
    const yAxisStops: [number, string][] = ((this.config as any)._yAxis ?? (this.config as any).yAxis)?.stops || [];
