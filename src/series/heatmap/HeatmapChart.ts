...
export class HeatmapChart extends BaseSeries {
  ...
  render(): void {
    ...
    if (useInterpolation) {
      ...
      this.renderHeatmapLabels(data, getCellX, getCellY, cellWidth, cellHeight, getCellColor);
  ...
  private renderHeatmapLabels(data: any[], getCellX: (d: any) => number, getCellY: (d: any) => number, cellWidth: number, cellHeight: number, getCellColor: (d: any) => string): void {
    ...
    const fontColorOverride = dlCfg.color || (dlCfg.style?.color as string) || undefined;
    ...
    this.group.selectAll('.katucharts-heatmap-label')
      ...
      .attr('fill', (d: any) => readableTextColor(getCellColor(d), fontColorOverride))
