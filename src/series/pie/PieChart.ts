...
export class PieChart extends BaseSeries {
  ...
  init(context: SeriesContext): void {
    const hasLabels = this.dataLabelConfigs().length > 0;
    if (hasLabels) {
  ...
  render(): void {
            ...
            if (selectColor || selectBorderColor || selectBorderWidth !== undefined) {
              slices.each(function(sd: any, si: number) {
                ...
                if (self.selectedIndices.has(si)) {
                  ...
                  el.attr('stroke', self.config.borderColor || self.autoBorderColor());
    ...
    /**
     * Render every data-label set — several are allowed per point (e.g. the
     * name outside the slice plus the percentage inside it). Each set declutters
     * itself: outside sets via vertical distribution, inside sets by hiding the
  ...
  private renderPieLabels(
    ...
    if (!isInside && !dlCfg.allowOverlap) {
      ...
      /**
       * Inside labels sit on their slice's centroid, and thin slices push those
       * centroids together near the middle. Keep the higher-value labels and hide
       * any that would overlap one already kept, so small slices drop their label
       * instead of stacking an unreadable clump of percentages at the centre.
       */
      ...
      for (const l of ordered) {
        ...
        const clash = kept.some(k => box.l < k.r && box.r > k.l && box.t < k.b && box.b > k.t);
        if (clash) { l.visible = false; continue; }
        kept.push(box);
      }
    }
  ...
  private distribute(
    ...
    /**
     * Cap how far a label may slide from its slice before it is dropped, so
     * heavily clustered regions thin out (the smallest slices there lose their
     * labels) instead of fanning every label across the plot on long leaders.
     */
    const maxDisplacement = labelHeight * 3;
    this.distributeBoxes(boxes, originalLength, maxDisplacement);
  ...
  private distributeBoxes(
    ...
    /**
     * Capacity cull: drop the lowest-ranked labels that cannot fit the vertical
     * span at all. The length stays fixed across this whole routine — unlike the
     * old recursion it never shrinks, so culling cannot cascade.
     */
    ...
    /**
     * Pack the survivors, then drop the lowest-ranked label that packing forced
     * more than `maxDisplacement` from its slice and re-pack. Removing one label
     * relaxes its neighbours, so this converges to the largest set that fits
     * without any label drifting too far — clusters thin, sparse regions keep
     * every label.
     */
    for (let guard = 0; guard <= boxes.length; guard++) {
      ...
      /**
       * Greedy top-down placement at each label's ideal position, pushed down to
       * clear the one above, then clamped up from the bottom so the stack stays
       * in view. Since the survivors fit, this leaves no overlaps.
       */
      ...
      /**
       * Relaxation: nudge each label back toward its slice without reintroducing
       * overlap, so labels sit as close to their target as the packing allows.
       */
      ...
      let worst: typeof alive[0] | null = null;
      for (const b of alive) {
        if (Math.abs(b.pos - b.target) <= maxDisplacement) continue;
