...
export class CircosChart extends BaseSeries {
  ...
  render(): void {
    ...
    engine.renderIdeogram(
      ...
      !!animate, ideogramDur,
      this.context.events, this, this.autoBorderColor(),
    );
    ...
    const labelColor = this.autoLabelColor();
    ...
    if (showLabels) {
      ...
      engine.renderChromosomeLabels(
        ...
        !!animate, ideogramDur, labelDelay, labelColor,
      );
    }
    ...
    /**
     * Flow tracks (ribbon, link) animate after regular tracks
     * so they render as foreground connectors atop other elements.
     */
    const isFlowTrack = (t: any) => t.type === 'ribbon' || t.type === 'link';
    let regularAnimIdx = 0;
    let flowAnimIdx = nTracks;
    ...
    for (let ti = 0; ti < nTracks; ti++) {
      ...
      if (track.axes) {
        renderTrackAxes(trackGroup, track.axes, actualInner, actualOuter, labelColor);
      }
      ...
      const renderOpts: TrackRenderOptions = {
        ...
        seriesRef: this,
        separatorColor: this.autoBorderColor(),
      };
