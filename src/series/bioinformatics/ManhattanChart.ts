...
export class ManhattanChart extends BaseSeries {
  ...
  private renderChromosomeLabels(data: PointOptions[]): void {
    ...
    /**
     * Chromosome labels stand in for suppressed numeric x-axis tick labels, rendered identically
     * to axis ticks: matching colour and size from `labels.style`, but generic sans-serif family
     * to avoid visual mismatch with the y-axis labels.
     */
    const labelStyle = (this.context.xAxis as any)?.config?.labels?.style || {};
    ...
    /**
     * plotGroup persists across reflows/re-renders (unlike this.group, which core clears), so
     * remove prior labels before each redraw to avoid stacking duplicates that render as bold text.
     */
    const parentGroup = this.context.plotGroup || this.group;
    parentGroup.selectAll('g.katucharts-manhattan-chr-labels').remove();
