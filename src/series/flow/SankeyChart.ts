...
export class SankeyChart extends BaseSeries {
  ...
  render(): void {
    ...
    const dlColor = dlCfg.color || (dlCfg.style?.color as string) || this.autoLabelColor();
    ...
    const leftLabelPad = dlEnabled && isNarrow
      ...
      : 0;
    /**
     * Last-column node labels render inside the plot — anchored to the left edge
     * of their rects and extending into the link gap — rather than outside the
     * diagram. No external right gutter is reserved, so the sankey fills the full
     * plot width and the last column sits flush right, mirroring the first column.
     */
    const dedicatedLastLabels = dlEnabled && !isUltraNarrow;
    const rightLabelPad = 0;
    ...
    /**
     * Shrinks node padding when nodes would overflow
    ...
    /**
     * Label-friendly gaps: d3-sankey couples padding and node size — a larger
     * nodePadding rescales every node (and its links) smaller to keep the column
     * inside plotArea.height. When labels are on and the padding is auto, widen
     * the gaps to a full label line-height so adjacent labels clear each other:
     * the tallest flows shrink proportionally and each small node gains a gap big
     * enough to hold its own label. This is only done when that gap genuinely
     * fits the densest column — if it cannot, the column is too dense to label
     * every node, so the layout is left natural (nodes stay value-proportional)
     * and the overlap pass below simply hides the labels with no room, rather
     * than squeezing every node to nothing chasing labels that still won't fit.
     */
    const autoNodePadding = typeof cfg.nodePadding !== 'number';
    if (dlEnabled && autoNodePadding && maxNodesInColumn > 1) {
      const labelGap = effectiveFontPx * 1.3;
      const maxAffordablePad = (plotArea.height * 0.82) / (maxNodesInColumn - 1);
      if (maxAffordablePad >= labelGap) {
        effectiveNodePadding = Math.max(baseNodePadding, labelGap);
      }
    }
    /**
     * Crowding floor: if even pure padding cannot seat every node, fall back to
     * an even split so nothing overflows the plot vertically.
     */
    if (maxNodesInColumn * effectiveNodePadding > plotArea.height) {
      effectiveNodePadding = Math.max(1, plotArea.height / maxNodesInColumn);
    }
    if (effectiveNodePadding !== baseNodePadding) {
    ...
    if (hasExplicitColumns) {
      ...
      this.reorderNodesByValueDesc(graph, sankeyGen, getNodeColumn, plotArea.height, effectiveNodePadding);
    }
    ...
    /**
     * Forward-declared so the hover handlers (defined below, before the
     * dlEnabled block) can fade labels via closure once they are mounted.
     */
    let labelNodes: any = null;
    const FADED_OPACITY = 0.55;
    const FADED_LABEL_OPACITY = 0.25;
    ...
    const handleLinkMouseOver = (event: MouseEvent, d: any) => {
      ...
      labelNodes?.interrupt('highlight');
      ...
      nodeRects.transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER)
        .attr('opacity', (n: any) => n === d.source || n === d.target ? 1 : FADED_OPACITY);
      labelNodes?.transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER)
        .attr('opacity', (n: any) => n === d.source || n === d.target ? 1 : FADED_LABEL_OPACITY);
    ...
    const handleLinkMouseOut = (event: MouseEvent, d: any) => {
      ...
      labelNodes?.transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER).attr('opacity', 1);
    ...
    /**
     * Shared handlers so labels (bound below) get the same node-hover behaviour
     * as the rects — important on dense charts where the rect is a few pixels
     * tall and the label is the only practical hit target.
     */
    const handleNodeMouseOver = (event: MouseEvent, d: any) => {
      nodeRects.filter((n: any) => n === d).style('filter', 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))');
      nodeRects.interrupt('highlight');
      linkPaths.interrupt('highlight');
      labelNodes?.interrupt('highlight');
      /**
       * A node is "connected" to the hovered one when a link joins them; keeping
       * those rects/labels at full opacity lets the user trace the relationship
       * visually instead of guessing from the highlighted curves alone.
       */
      const connected = new Set<any>([d]);
      for (const l of graph.links) {
        if (l.source === d) connected.add(l.target);
        else if (l.target === d) connected.add(l.source);
      }
      nodeRects.attr('opacity', 1);
      nodeRects.filter((n: any) => !connected.has(n)).transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER).attr('opacity', FADED_OPACITY);
      labelNodes?.transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER)
        .attr('opacity', (n: any) => connected.has(n) ? 1 : FADED_LABEL_OPACITY);
      linkPaths.transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER)
    ...
    const handleNodeMouseOut = (event: MouseEvent, d: any) => {
      nodeRects.filter((n: any) => n === d).style('filter', '');
      nodeRects.interrupt('highlight');
      linkPaths.interrupt('highlight');
      labelNodes?.interrupt('highlight');
      nodeRects.transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER).attr('opacity', 1);
    ...
    if (dlEnabled) {
      ...
      const minNodeX0Render = graph.nodes.reduce((m: number, n: any) => Math.min(m, n.x0 ?? Infinity), Infinity);

      /**
       * Initial placement: leaf-column nodes (first or last) get a dedicated
       * label that hugs the rect and extends inward into the link gap — the
       * first column anchors `start` at x1+4 (text flows right), the last column
       * anchors `end` at x0-4 (text flows left). Both stay inside the plot.
       * Middle-column nodes use `middle` anchor centred on the rect.
       */
      const isLastCol = (d: any) => hasExplicitColumns
        ? getNodeColumn(d) === maxDepth
        : maxNodeX0 > 0 && (d.x0 ?? 0) >= maxNodeX0 - 0.5;
      const isFirstCol = (d: any) => hasExplicitColumns
        ? getNodeColumn(d) === 0
        : Number.isFinite(minNodeX0Render) && (d.x0 ?? 0) <= minNodeX0Render + 0.5;
      const shouldUseDedicatedLabel = (d: any) =>
        (dedicatedLastLabels && isLastCol(d)) || isFirstCol(d);

      /**
       * Pick label text per node, honouring formatter + min-height.
       * Dedicated leaf-column labels extend past the rect into the link gap, so
       * node thickness is irrelevant — skip the min-height clear for them. The
       * downstream overlap pass below culls collisions by value priority.
       */
      const labelTextByNode = new Map<any, string>();
      for (const d of graph.nodes) {
        const nodeHeight = d.y1 - d.y0;
        if (!shouldUseDedicatedLabel(d) && nodeHeight < minNodeHeightForLabel) {
      ...
      /**
       * First column (sources) flows right into the link gap (`start` anchor at
       * x1+4); last column (sinks) flows left into the link gap (`end` anchor at
       * x0-4) so its labels stay inside the plot instead of off the right edge.
       * Middle-column nodes use `inside: true` placement (middle-
       * anchor, centred), letting the text overflow narrow rects.
       */
      const initialAnchorFor = (d: any) =>
        shouldUseDedicatedLabel(d) ? (isLastCol(d) ? 'end' : 'start') : 'middle';
      const initialXFor = (d: any) => {
        if (!shouldUseDedicatedLabel(d)) return (d.x0 + d.x1) / 2 + (dlCfg.x ?? 0);
        return isLastCol(d) ? d.x0 - 4 + (dlCfg.x ?? 0) : d.x1 + 4 + (dlCfg.x ?? 0);
      };

      labelNodes = labelsGroup.selectAll('.katucharts-sankey-label')
        .data(graph.nodes)
        .join('text')
        ...
        .attr('stroke', (dlCfg.style as any)?.textOutline ? null : this.labelHaloColor())
      ...
      /**
       * Contrast outline (a standard halo that opposes the label so it reads on
       * any fill — dark behind light text, light behind dark — instead of a fixed
       * white that blows out on dark themes).
       *
       * Justify pass: if a middle-anchored label's bbox overflows a plot edge,
       * re-anchor it on the node side with more room (mirrors the
      ...
      labelNodes.each(function(this: any, d: any) {
        ...
        if (shouldUseDedicatedLabel(d)) {
          /**
           * First column extends right into the link gap, last column extends left.
           * Cap each at the midpoint of the gap to the neighbouring column (less a
           * small spacing) so the two end columns' labels keep clear of each other
           * and never run up against the opposite column's nodes. Same-side
           * collisions are still culled by the overlap pass below.
           */
          const dep = getNodeColumn(d);
          const spacing = effectiveFontPx * 0.75;
          let avail: number;
          if (isLastCol(d)) {
            const prevRight = colMax.get(dep - 1);
            const mid = prevRight !== undefined ? (prevRight + d.x0) / 2 : 2;
            avail = (d.x0 - 4) - (mid + spacing / 2);
          } else {
            const nextLeft = colMin.get(dep + 1);
            const mid = nextLeft !== undefined ? (d.x1 + nextLeft) / 2 : plotArea.width - 2;
            avail = (mid - spacing / 2) - (d.x1 + 4);
          }
      ...
      /**
       * Value-priority overlap hiding: iterate labels in descending value
  ...
  private applyExplicitColumnLayout(
    ...
    for (const [column, colNodes] of orderedColumns) {
      /**
       * Default to value-descending so the highest-volume nodes sit at the top —
       * gives a consistent visual hierarchy and lets the label overlap pass keep
       * the most informative ones. Honor explicit `offset` whenever the user
       * sets one on any node in the column.
       */
      const hasExplicitOffset = colNodes.some((n: any) => typeof n.offset === 'number');
  ...
  private reorderNodesByValueDesc(
    ...
    for (const [, colNodes] of columns) {
      const hasExplicitOffset = colNodes.some((n: any) => typeof n.offset === 'number');
      if (hasExplicitOffset) continue;

      const totalNodeHeight = colNodes.reduce((sum: number, n: any) => sum + (n.y1 - n.y0), 0);
      const totalUsed = totalNodeHeight + Math.max(0, colNodes.length - 1) * nodePadding;
      ...
      let y = Math.max(0, (plotHeight - totalUsed) / 2);
      for (const node of colNodes) {
