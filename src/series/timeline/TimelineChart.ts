...
export class TimelineChart extends BaseSeries {
  ...
  render(): void {
    ...
    data.forEach((d, i) => {
      ...
      /**
       * Hover effects for timeline event and attached event listeners.
       */
      ...
      const onOver = (event: MouseEvent) => {
        ...
        rows.forEach(r => {
          ...
          if (r.desc) {
            r.desc.interrupt('highlight');
            r.desc.attr('opacity', 1);
          }
        });
        rows.forEach((r, j) => {
          if (j !== i) {
            ...
            if (r.desc) {
              r.desc.transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER).attr('opacity', 0.4);
            }
          }
        });
      ...
      const onOut = (event: MouseEvent) => {
        ...
        rows.forEach(r => {
          ...
          if (r.desc) {
            r.desc.interrupt('highlight');
            r.desc.transition('highlight').duration(HOVER_DURATION).ease(EASE_HOVER).attr('opacity', 1);
          }
        });
