...
export class LineChart extends BaseSeries {
  ...
  protected buildLineGenerator() {
    ...
    const xVal = (d: any, i: number) => (typeof d === 'number' ? i : (d?.x ?? i));
    const yVal = (d: any) => (typeof d === 'number' ? d : d?.y);

    const gen = line<PointOptions>()
      .x((d, i) => inverted ? yAxis.getPixelForValue(yVal(d) ?? 0) : xAxis.getPixelForValue(xVal(d, i)))
      .y((d, i) => inverted ? xAxis.getPixelForValue(xVal(d, i)) : yAxis.getPixelForValue(yVal(d) ?? 0))
      .curve(this.getCurve());

    if (!connectNulls) {
      gen.defined(d => { const y = yVal(d); return y !== null && y !== undefined; });
    }
