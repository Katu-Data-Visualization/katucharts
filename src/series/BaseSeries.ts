...
export abstract class BaseSeries {
  ...
  protected autoLabelColor(override?: string): string {
    return readableTextColor(this.context.backgroundColor, override);
  }
  ...
  protected labelHalo(): string {
    const halo = this.labelHaloColor();
    return `0 0 3px ${halo}, 0 0 3px ${halo}`;
  }

  /** Single halo color (for SVG `stroke` outlines) that contrasts with the label. */
  protected labelHaloColor(): string {
    return readableTextColor(this.context.backgroundColor) === '#ffffff' ? 'rgba(0,0,0,0.85)' : '#ffffff';
  }
  ...
  protected autoBorderColor(): string {
    const bg = this.context.backgroundColor;
  ...
  getBubbleLegendInfo(): {
    sizeBy: 'area' | 'width';
    color: string;
    ranges: { value: number; radius: number }[];
  } | null {
    return null;
  }
