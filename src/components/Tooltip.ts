...
import { templateFormat, dateFormat, escapeHtml, sanitizeColor } from '../utils/format';
...
export class Tooltip {
  ...
  private sharedProvider: ((x: number | string) => TooltipPointData[]) | null = null;

  private static readonly DEFAULT_DATE_FORMAT = '%A, %b %e, %Y';
  ...
  show(rawData: any): void {
    ...
    if (this.config.shared) {
      ...
      if (this.sharedProvider) {
        /**
         * Resolve every series' point at the hovered x, so one hover shows the
         * whole column (not only the series physically under the cursor).
         */
        const resolved = this.sharedProvider(currentX);
        this.sharedPoints = resolved.length > 0 ? resolved : [data];
      } else {
  ...
  private formatContent(data: TooltipPointData): string {
    ...
    const xDateFmt = seriesTooltip?.xDateFormat ?? this.config.xDateFormat
      ?? (this.isDatetime ? Tooltip.DEFAULT_DATE_FORMAT : undefined);
    if (xDateFmt && typeof data.point.x === 'number' && data.point.name === undefined) {
    ...
    if (pointFormatter) {
      ...
      return templateFormat(headerFmt, context, true) + pointBody + templateFormat(footerFmt, context, true);
    }

    if (this.config.format) {
      return templateFormat(this.config.format, context, true);
    }
    ...
    const header = templateFormat(headerFmt, context, true);
    const body = templateFormat(pointFmt, context, true);
    const footer = templateFormat(footerFmt, context, true);
  ...
  private formatSharedContent(points: TooltipPointData[]): string {
    ...
    const headerKey = this.formatXKey(points[0]?.point, points[0]?.series.config?.tooltip);
    if (headerKey !== '') {
      parts.push(`<span style="font-size:10px">${escapeHtml(String(headerKey))}</span><br/>`);
    }

    for (const p of points) {
      const seriesTooltip = p.series.config?.tooltip as TooltipOptions | undefined;
      const seriesPointFmt = seriesTooltip?.pointFormat;
      if (seriesPointFmt) {
        const context = {
          point: this.buildFormatterPoint(p),
          series: { ...p.series, name: p.series.name || '' },
        };
   ...
   */
  private formatXKey(point: PointOptions | undefined, seriesTooltip?: TooltipOptions): string | number {
    if (!point) return '';
    if (this.categories && typeof point.x === 'number' && this.categories[point.x] !== undefined) {
      return this.categories[point.x];
    }
    if (point.name !== undefined && point.name !== null) return point.name as string | number;
    const x = point.x;
    if (typeof x === 'number') {
      const xDateFmt = seriesTooltip?.xDateFormat ?? this.config.xDateFormat
        ?? (this.isDatetime ? Tooltip.DEFAULT_DATE_FORMAT : undefined);
      if (xDateFmt) return dateFormat(xDateFmt, x);
    }
    return x ?? '';
  }
