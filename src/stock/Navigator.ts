...
export class Navigator {
  ...
  private brushGroup: Selection<SVGGElement, unknown, null, undefined> | null = null;
  ...
  setSelection(x0Frac: number, x1Frac: number): void {
    if (!this.brush || !this.brushGroup) return;
    this.programmatic = true;
    if (x0Frac <= 0.0005 && x1Frac >= 0.9995) {
      this.brush.move(this.brushGroup as any, null);
    } else {
      const a = Math.max(0, Math.min(1, x0Frac)) * this.width;
      const b = Math.max(0, Math.min(1, x1Frac)) * this.width;
      this.brush.move(this.brushGroup as any, [Math.min(a, b), Math.max(a, b)]);
    }
