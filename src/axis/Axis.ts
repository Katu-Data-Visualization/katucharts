...
import {
  scaleLinear, scaleLog, scaleTime, scaleUtc, scaleBand, scalePoint, scaleSequential,
...
class BaseAxis {
  ...
  protected autoText(): string {
    return readableTextColor(this.config._backgroundColor);
  }
  ...
  protected getRange(): [number, number] {
    ...
    const band = this.getPaneBand();
    const lo = band.top;
    const hi = band.top + band.height;
    return this.config.reversed ? [lo, hi] : [hi, lo];
  }
   ...
   */
  protected getPaneBand(): { top: number; height: number } {
    const full = this.plotArea.height;
    const toPx = (v: number | string | undefined): number | null => {
      if (v == null) return null;
      if (typeof v === 'number') return v;
      if (typeof v === 'string' && v.trim().endsWith('%')) return (parseFloat(v) / 100) * full;
      const n = parseFloat(v as string);
      return Number.isFinite(n) ? n : null;
    };
    const top = toPx(this.config.top) ?? 0;
    const height = toPx(this.config.height) ?? (full - top);
    return { top, height };
  }
    ...
    for (const band of this.config.plotBands) {
      ...
      if (band.label?.text) {
        ...
        group.append('text')
          ...
          .attr('fill', (lbl.style as any)?.color || this.autoText())
  ...
  protected renderTitle(
    ...
    if (rendersHorizontally) {
      ...
      const el = group.append('text')
        ...
        .attr('fill', title.style?.color as string || this.autoText())
        .text(title.text!);
      ...
      const band = this.getPaneBand();
      const yPos = band.top + band.height / 2;
...
export class DateTimeAxis extends BaseAxis implements AxisInstance {
  ...
  constructor(config: InternalAxisConfig, plotArea: PlotArea) {
    super(config, plotArea);
    const makeScale = getUseUTC() ? scaleUtc : scaleTime;
    this.scale = makeScale().range(this.getRange()).domain([new Date(), new Date()]) as any;
  }
  ...
  render(group: Selection<SVGGElement, unknown, null, undefined>, plotArea: PlotArea): void {
    ...
    if (this.config.labels?.formatter) {
      ...
      const tf = getUseUTC() ? utcFormat : timeFormat;
  ...
  private pickDateFormat(date: Date, fmts: Record<string, string>): string {
    ...
    const sec = utc ? date.getUTCSeconds() : date.getSeconds();
    const min = utc ? date.getUTCMinutes() : date.getMinutes();
    const hr = utc ? date.getUTCHours() : date.getHours();
    const dom = utc ? date.getUTCDate() : date.getDate();
    const mon = utc ? date.getUTCMonth() : date.getMonth();
    if (ms !== 0 && fmts.millisecond) return fmts.millisecond;
    if (sec !== 0 && fmts.second) return fmts.second;
    if (min !== 0 && fmts.minute) return fmts.minute;
    if (hr !== 0 && fmts.hour) return fmts.hour;
    if (dom !== 1 && fmts.day) return fmts.day;
    if (mon !== 0 && fmts.month) return fmts.month;
...
export class CategoryAxis extends BaseAxis implements AxisInstance {
  ...
  render(group: Selection<SVGGElement, unknown, null, undefined>, plotArea: PlotArea): void {
    ...
    if (isNaN(range[0]) || isNaN(range[1]) || range[0] === range[1]) return;
