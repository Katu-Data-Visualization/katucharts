...
export class LayoutEngine {
  compute(config: InternalConfig, chartWidth: number, chartHeight: number): LayoutResult {
    ...
    const legendAlign = config.legend?.align || 'center';
    /**
     * A vertical legend pinned to the left/right edge is reserved as a side
     * strip (carving horizontal space) instead of the default top/bottom band.
     */
    const isSideLegend = !hasHeatmap
      && config.legend?.enabled !== false
      && config.legend?.layout === 'vertical'
      && (legendAlign === 'left' || legendAlign === 'right');

    /**
     * For heatmaps, the color axis serves as the legend and is rendered within the plot group.
     */
    const legendHeight = (hasHeatmap || isSideLegend) ? 0 : this.estimateLegendHeight(config, chartWidth);
    const legendPosition = config.legend?.verticalAlign || 'bottom';
    if (!isSideLegend && legendPosition === 'bottom') {
      bottom = Math.max(bottom, spacing.bottom) + legendHeight;
    } else if (!isSideLegend && legendPosition === 'top') {
      top += legendHeight;
    }
    const sideLegendWidth = isSideLegend ? this.estimateLegendWidth(config) : 0;
    const leftLegendWidth = isSideLegend && legendAlign === 'left' ? sideLegendWidth : 0;
    const rightLegendWidth = isSideLegend && legendAlign === 'right' ? sideLegendWidth : 0;

    /**
     * Reserve room below the plot (and x-axis) for the stock navigator.
     */
    if ((config as any).navigator?.enabled) {
      const nav = (config as any).navigator;
    ...
    const plotY = top;
    const plotWidth = Math.max(0, chartWidth - left - right - yAxisLeftWidth - yAxisRightWidth - colorAxisRightWidth - leftLegendWidth - rightLegendWidth);
    ...
    return {
      ...
      legendArea: isSideLegend
        ? {
            x: legendAlign === 'right' ? chartWidth - right - rightLegendWidth : left,
            y: plotY,
  ...
  private isNonCartesian(config: InternalConfig): boolean {
    return config.series.length > 0 &&
      config.series.every(s => NO_AXES_TYPES.has(s._internalType));
  }
  ...
  private estimateLegendHeight(config: InternalConfig, chartWidth: number): number {
    const base = this.estimateSeriesLegendHeight(config, chartWidth);
    if (config.legend?.enabled && config.legend.bubbleLegend?.enabled) {
      const bubble = config.series.find(s => s._internalType === 'bubble');
      if (bubble) {
        const margin = config.legend.margin ?? 8;
        const maxR = config.legend.bubbleLegend.maxSize
          ?? (typeof bubble.maxSize === 'number' ? bubble.maxSize : 30);
        const bubbleCluster = maxR * 2 + 16 + margin;
        /**
         * The 'separate' layout stacks the size cluster *beneath* the series
         * row, so reserve room for both; otherwise they sit side by side.
         */
        const isSeparate = (config.legend.layout || 'horizontal') === 'horizontal'
          && config.legend.bubbleLegend.layout === 'separate';
        return isSeparate ? base + bubbleCluster : Math.max(base, bubbleCluster);
      }
    }
    return base;
  }
   ...
   */
  private estimateLegendWidth(config: InternalConfig): number {
    const legend = config.legend;
    if (!legend) return 0;
    const visibleSeries = config.series.filter(s => s.showInLegend !== false);
    const itemStyle = legend.itemStyle || {};
    const fontPx = parseFontSizePx(itemStyle.fontSize as string || DEFAULT_CHART_TEXT_SIZE);
    const symbolWidth = legend.symbolWidth ?? 12;
    const symbolPadding = legend.symbolPadding ?? 5;
    const padding = legend.padding ?? 8;
    ...
    if (legend.bubbleLegend?.enabled) {
      const bubble = config.series.find(s => s._internalType === 'bubble');
      if (bubble) {
        const maxR = legend.bubbleLegend.maxSize
          ?? (typeof bubble.maxSize === 'number' ? bubble.maxSize : 30);
        const connectorDistance = legend.bubbleLegend.connectorDistance ?? 16;
        /**
         * Width required for bubble legend: diameter (2*maxR) + connector + label padding.
         */
        width = Math.max(width, maxR * 2 + connectorDistance + 30);
      }
    }

    const margin = legend.margin ?? 12;
    return width + padding * 2 + margin;
  }

  private estimateSeriesLegendHeight(config: InternalConfig, chartWidth: number): number {
    ...
    if (layout === 'horizontal') {
      ...
      for (let i = 0; i < labels.length; i++) {
        const textWidth = config.legend.itemWidth || measureTextWidth(labels[i], fontPx);
        const itemWidth = symbolWidth + symbolPadding + textWidth;
        naturalFlowWidth += itemWidth + (i < labels.length - 1 ? itemDistance : 0);
      }
  ...
  private estimateAxisWidth(config: InternalConfig, isLeft: boolean, chartHeight?: number, chartWidth?: number): number {
    ...
    for (const axis of axes) {
      ...
      const labelWidth = hasLabels ? this.estimateLabelWidth(axis, chartHeight, chartWidth) : 0;
      width += labelWidth + (hasTitle ? 30 : 0) + (axis.offset || 0);
    }
    return width || 30;
  }

  private estimateLabelWidth(axis: InternalConfig['yAxis'][0], chartHeight?: number, chartWidth?: number): number {
    if (axis.categories && axis.categories.length > 0) {
      const configFontSize = parseFontSizePx(axis.labels?.style?.fontSize as string || DEFAULT_CHART_TEXT_SIZE);
      let renderedFontSize = configFontSize;
      if (chartHeight && axis.categories.length > 1) {
        const roughPlotHeight = chartHeight * 0.8;
        const step = roughPlotHeight / axis.categories.length;
        const bandwidth = step * 0.7;
        if (configFontSize > bandwidth) {
          renderedFontSize = Math.min(configFontSize, Math.max(5, Math.round(bandwidth * 1.1)));
        }
      }
      const fontWeight = (axis.labels?.style?.fontWeight as string) || 'normal';
      let maxWidth = 0;
      for (const cat of axis.categories) {
        const w = measureTextWidth(cat, renderedFontSize, fontWeight);
